...
function createCharts() {
    ...
    // Add legend items for categories with counts greater than or equal to 2
    sortedLabels.forEach((label, index) => {
        if (sortedValues[index] >= 2) {
    ...
    const hiddenProjects = sortedValues.filter(value => value < 2);
    if (hiddenProjects.length > 0) {
        ...
        expandButton.onclick = function () {
            ...
            seeLessButton.onclick = function () {
                ...
                // Re-add legend items for categories with counts greater than or equal to 2
                sortedLabels.forEach((label, index) => {
                    if (sortedValues[index] >= 2) {
                        const legendItem = document.createElement('div');
                        legendItem.classList.add('chart-legend-sector-item');
